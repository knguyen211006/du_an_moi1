...
export default function QuizPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const saveWrongAnswerToNotebook = async (word: QuizQuestion) => {
    ...
    try {
      await addWordToNotebook(currentUser.uid, {
        hanzi: word.hanzi,
        pinyin: word.pinyin,
  ...
  if (isFinished) {
    ...
    return (
      <div className="min-h-screen bg-[#09060f] text-slate-100 pt-24 pb-12 px-4 relative overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-cyan-500/10 rounded-full blur-[128px]" />
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-purple-500/10 rounded-full blur-[128px]" />
        <div className="max-w-4xl mx-auto relative z-10 flex flex-col items-center min-h-[calc(100vh-8rem)] space-y-8">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-orange-400 to-yellow-400 tracking-widest text-center">
          ...
          <div className="w-full max-w-lg bg-gradient-to-br from-amber-500/20 to-orange-500/10 backdrop-blur-xl border border-amber-500/20 rounded-3xl p-8 shadow-2xl text-center">
            <div className="text-6xl md:text-7xl font-black text-neutral-200 mb-2">
          ...
          {wrongAnswers.length > 0 && (
            <div className="w-full max-w-lg bg-gradient-to-br from-red-500/10 to-orange-500/5 backdrop-blur-xl border border-red-500/20 rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-4 text-red-400">
                ...
                <span className="ml-auto text-xs bg-red-500/20 px-2 py-1 rounded-full">{wrongAnswers.length}</span>
              </div>
              <div className="space-y-3 max-h-60 overflow-y-auto">
                {wrongAnswers.map((word, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5 hover:border-red-500/30 transition-all">
                    <div>
          ...
          <div className="w-full max-w-lg space-y-4">
            <Link href="/roadmap" className="w-full py-4 px-8 bg-gradient-to-r from-cyan-600 to-purple-600 hover:from-cyan-500 hover:to-purple-500 text-white font-bold text-lg rounded-2xl shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40 transition-all transform hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-3">
              <Home size={20} />
              Về Lộ Trình
            </Link>
            <button onClick={handleRestart} className="flex items-center justify-center gap-2 w-full py-4 px-6 rounded-2xl border border-amber-500/30 text-amber-400 hover:bg-amber-500/10 hover:border-amber-500/50 transition-all duration-300 backdrop-blur-sm font-bold text-lg">
              <RotateCcw size={20} />
              Luyện lại từ đầu
            </button>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#09060f] text-slate-100 pt-24 pb-12 px-4 relative overflow-hidden">
      {isWrongAnimation && <div className="fixed inset-0 pointer-events-none z-50 animate-screen-flash-red" />}
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-cyan-500/10 rounded-full blur-[128px]" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-purple-500/10 rounded-full blur-[128px]" />
      <div className="max-w-4xl mx-auto relative z-10 flex flex-col min-h-[calc(100vh-8rem)]">
        <div className="flex items-center justify-between mb-8 gap-4">
          <Link href={`/lessons/${resolvedParams.id}`} className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl transition-all text-slate-300 hover:text-cyan-400 text-sm font-medium">
            <X size={16} />
          ...
          <div className="flex-1 max-w-md mx-auto">
            ...
            <div className="h-2.5 bg-white/10 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-cyan-500 to-purple-500 transition-all duration-700 ease-out rounded-full shadow-[0_0_10px_rgba(6,182,212,0.4)] animate-progress-glow" style={{ width: `${progressPercentage}%` }} />
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={toggleSound} className="p-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl transition-all text-slate-400 hover:text-cyan-400" title={isSoundEnabled ? "Tắt âm thanh" : "Bật âm thanh"}>
          ...
          <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-4">
            {currentQ.options.map((option, index) => {
              ...
              return (
                ...
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm mr-4 transition-colors ${labelStyle}`}>
                    {OPTION_LABELS[index]}
                  </div>
                  <span className="font-semibold text-lg">{option}</span>
                  {hasSubmitted && isCorrect && <CheckCircle2 className="absolute right-4 text-green-400" size={24} />}
                  {hasSubmitted && isSelected && !isCorrect && <XCircle className="absolute right-4 text-red-400" size={24} />}
                </button>
              )
            })}
          </div>
          {hasSubmitted && (
            <div className={`w-full p-6 rounded-2xl border animate-fade-in ${selectedOption === currentQ.correctAnswer ? "bg-emerald-500/10 border-emerald-500/20" : "bg-red-500/10 border-red-500/20"}`}>
              <h3 className={`font-bold mb-2 flex items-center gap-2 ${selectedOption === currentQ.correctAnswer ? "text-emerald-400" : "text-red-400"}`}>
                {selectedOption === currentQ.correctAnswer ? <CheckCircle2 size={20} /> : <XCircle size={20} />}
                {selectedOption === currentQ.correctAnswer ? "Tuyệt vời! Đáp án chính xác." : "Rất tiếc! Đáp án chưa chuẩn."}
              </h3>
              ...
              {selectedOption !== currentQ.correctAnswer && (
                <div className="mt-3 flex items-center gap-2 text-sm">
                  ...
                  {notebookSaveStatus === "saving" && <span className="text-amber-400">Đang khắc vào Sổ Thù Hận...</span>}
                  {notebookSaveStatus === "saved" && <span className="text-green-400">Đã khắc vào Sổ Thù Hận!</span>}
                  {notebookSaveStatus === "error" && <span className="text-red-400">Lỗi lưu Sổ Thù Hận</span>}
                </div>
              )}
            </div>
          )}
